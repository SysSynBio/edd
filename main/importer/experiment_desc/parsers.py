...
ICE_PART_PATTERN = re.compile(r'^\s*([A-Z]+_[A-Z]?\d{4,6}[A-Z]?)\s*$', re.IGNORECASE)
...
LINE_NAME_COL_LABEL = r'Line\s+Name'
LINE_DESCRIPTION_COL_REGEX = r'Line\s+Description'
STRAIN_IDS_COL_LABEL = r'Part\s+ID'
REPLICATE_COUNT_COL_REGEX = r'Replicate\s+Count'
...
_STRAIN_GROUP_REGEX = r'^\s*\(((?:\s*[^' + _STRAIN_GROUP_MEMBER_DELIM + r'\)\(]+\s*' + \
                      _STRAIN_GROUP_MEMBER_DELIM + r'?\s*)+)\)\s*$'
...
class ExperimentDescFileParser(CombinatorialInputParser):
    ...
    def _check_part_id_pattern(self, part_id, input_location=''):
        ...
        # user input
        part_number_match = ICE_PART_PATTERN.match(part_id)
