...
class Peak(object):
  ...
  def __str__(self):
    return "%6d  %8.3f  %8d  %8d" % (self.number, self.retention_time,
      self.peak_height, self.peak_area)
...
class SampleCollection(object):
    ...
    for i_sample, sample in enumerate(self.samples):
      ...
      for i_peak, rt in enumerate(peak_times):
        ...
        if n_peaks == 0:
          ...
          if n_peaks > 1:
            print("WARNING: %d peaks near %.3f for sample %s" % (n_peaks, rt, sample.sample_id),
                  file=err)
            all_peaks = sample.get_peaks_around(rt, bandwidth)
            errors.append((i_sample, i_peak, "%d peaks found: %s" %
              (n_peaks, ", ".join(["%g" % x for x in all_peaks]))))
          row.append(area)
      table.append(row)
      unrecognized = sample.get_unrecognized_peaks(peak_times, bandwidth)
      if unrecognized:
        errors.append((i_sample, None, "Additional peaks: %s" %
  ...
  def show_peak_areas(self, n_expected=None, out=sys.stdout, err=sys.stderr):
    ...
    print(
      "          ID %s" % " ".join(["%12s" % ("Peak %d"%(k+1)) for k in range(len(table[0])-1)]),
      file=out
    )
    print(
