...
def process_import_file(import_pk, user_pk, requested_status, initial_upload):
    ...
    try:
        ...
        attempt_status_transition(import_, requested_status, user, notify=notify, async=False,
                                  aggregator=handler)
...
def attempt_status_transition(import_, requested_status, user, async, notify=None,
                              aggregator=None):
    """
    ...
    :param async True to attempt the import asynchronously in a separate Celery task, or False
    ...
    _verify_status_transition(aggregator, import_, requested_status, user, notify, async)

    if requested_status == Import.Status.SUBMITTED:
        submit_import(import_, user.pk, aggregator, notify, async)
