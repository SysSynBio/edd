...
export class LineNameFilterSection extends GenericFilterSection {
    ...
    setLineColors() {
        ...
        if (this.countChecked() === 0) {
            ...
            boxes.each((_, elem) => {
                let box = $(elem);
                let label = box.next('label');
                let line = this.lineLookup[box.attr('name')];
            ...
            boxes.each((_, elem) => {
                ...
                if (!box.prop('checked')) {
                    ...
                    line.color = this.lastColor = eddGraphing.colorQueue(this.lastColor);
                    label.css('color', line.color);
                } else {
...
function remakeMainGraphArea() {
    ...
    dataSets = postFilteringMeasurements.map((mId: number, i: number): GraphValue[] => {
        let measure: AssayMeasurementRecord = EDDData.AssayMeasurements[mId];
        let points: number = measure ? measure.values.length : 0;
        ...
        let assay: AssayRecord = EDDData.Assays[measure.assay];
        let line: LineRecord = EDDData.Lines[assay.lid];
    ...
    if (viewingMode === 'linegraph') {
        ...
        if (barGraphMode === 'time') {
            ...
            elem = $('#barGraphByLine').empty().removeClass('off').get(0);
        } else if (barGraphMode === 'measurement') {
            elem = $('#barGraphByMeasurement').empty().removeClass('off').get(0);
        } else {
            return;
        }
        let view = new GraphView(elem);
...
class DataGridSpecAssays extends DataGridSpecBase {
    ...
    getRecordIDs(): any[] {
        var lr = progressiveFilteringWidget.lastFilteringResults;
        if (lr) {
    ...
    findMaximumXValueInData(): void {
        ...
        minmax = this.getRecordIDs().reduce((outer: MinMax, assayId): MinMax => {
            ...
            if (assay.maxXValue !== undefined && assay.minXValue !== undefined) {
                ...
                recordMinmax = measures.reduce<MinMax>((middle: MinMax, measureId): MinMax => {
                    let lookup: RecordList<AssayMeasurementRecord>;
    ...
    private loadLineName(index: any): string {
        let assay = EDDData.Assays[index];
        if (assay) {
            let line = EDDData.Lines[assay.lid];
            if (line) {
